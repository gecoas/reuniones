...
const openRecordForm = (type, record = null) => {
  ...
  if (type === 'minutes') fields.innerHTML = `${record ? '' : `<label>Reunión y fecha<select name="meetingId" required>${options(adminData.meetings, '', item => `${item.title} · ${formatDate(item.starts_at)}`)}</select></label>`}<label>Asistentes<input name="attendees" value="${value('attendees')}" placeholder="Nombres separados por comas"></label><label>1. Resumen<textarea name="summary" maxlength="2000" placeholder="Resumen de la reunión (máximo cinco líneas)">${value('summary')}</textarea></label><label>2. Puntos tratados<textarea name="content" required placeholder="Se cargan desde el orden del día de la reunión; puedes completarlos.">${value('content')}</textarea></label><label>3. Acuerdos numerados<textarea name="agreements" placeholder="1. ...\n2. ...">${value('agreements')}</textarea></label><fieldset class="minute-task"><legend>4. Nueva tarea de seguimiento</legend><label>Qué<input name="taskTitle" placeholder="Tarea a realizar"></label><label>Quién<select name="taskAssigned"><option value="">Sin asignar</option>${options(adminData.users, '', user => user.name || user.email)}</select></label><label>Para cuándo<input name="taskDue" type="date"></label></fieldset><label>5. Pendientes para la próxima<textarea name="pending">${value('pending')}</textarea></label><label>Audio de la reunión (URL temporal)<input name="audioUrl" type="url" value="${value('audio_url')}" placeholder="Se preparará con Google Cloud Storage"></label><label>Estado<select name="status"><option value="draft" ${record?.status === 'draft' ? 'selected' : ''}>Borrador</option><option value="review" ${record?.status === 'review' ? 'selected' : ''}>En revisión</option><option value="sent" ${record?.status === 'sent' ? 'selected' : ''}>Enviada</option></select></label>`;
  ...
  if (type === 'minutes' && !record) fields.querySelector('[name="meetingId"]')?.addEventListener('change', async event => { const agenda = await api(`/api/meetings/${event.target.value}/agenda`); fields.querySelector('[name="content"]').value = agenda.map((item, index) => `${index + 1}. ${item.title}`).join('\n'); });
...
const saveRecordForm = async event => {
  ...
  if (type === 'members') {
    ...
    const meetingId = record?.meeting_id || data.meetingId; await api(`/api/meetings/${meetingId}/minutes`, { method: 'PATCH', body: JSON.stringify(data) });
    if (data.taskTitle) { const meeting = adminData.meetings.find(item => item.id === meetingId); await api('/api/tasks', { method: 'POST', body: JSON.stringify({ departmentId: meeting.department_id, meetingId, title: data.taskTitle, assignedTo: data.taskAssigned, dueDate: data.taskDue }) }); }
  } else {
...
document.querySelector('#newMeeting').addEventListener('click', async () => { const departments = await api(currentSession?.isAdmin ? '/api/departments' : '/api/my-departments'); const select = document.querySelector('#meetingDepartment'); select.innerHTML = options(departments, '', department => department.name); document.querySelector('#meetingDate').value = new Date().toISOString().slice(0, 10); document.querySelector('#meetingTime').value = '10:00'; modal.classList.add('open'); });
...
document.querySelector('#meetingForm').addEventListener('submit', async event => { event.preventDefault(); try { const departmentId = document.querySelector('#meetingDepartment').value; const title = document.querySelector('#meetingTitle').value; const startsAt = `${document.querySelector('#meetingDate').value}T${document.querySelector('#meetingTime').value}`; const meeting = await api('/api/meetings', { method: 'POST', body: JSON.stringify({ departmentId, title, startsAt, location: document.querySelector('#meetingLocation').value }) }); const items = document.querySelector('#meetingAgenda').value.split('\n').map(item => item.trim()).filter(Boolean); if (items.length) await api(`/api/meetings/${meeting.id}/agenda`, { method: 'POST', body: JSON.stringify({ items }) }); modal.classList.remove('open'); await loadDashboard(); showToast('Reunión creada'); } catch { showToast('No se pudo crear la reunión'); } });
