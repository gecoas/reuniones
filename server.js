...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.url?.match(/^\/api\/minutes\/[^/]+\/send$/) && request.method === 'POST') {
      ...
      const date = new Intl.DateTimeFormat('es-ES', { timeZone: 'Europe/Madrid', day: 'numeric', month: 'numeric', year: '2-digit' }).format(new Date(current.starts_at)); const subject = `Acta reunión Dpto. de ${current.department_name} (${date})`; const sections = [['Asistentes', current.attendees || 'No indicados'], ['Resumen', current.summary], ['Puntos tratados', current.content], ['Acuerdos', current.agreements], ['Pendientes para la próxima', current.pending]]; const html = `<h2>${escapeEmailHtml(subject)}</h2>${sections.map(([title, content]) => `<p><strong>${title}</strong><br>${escapeEmailHtml(content || 'Sin información')}</p>`).join('')}`; await transporter.sendMail({ from: SMTP_FROM, to: SMTP_FROM, bcc: recipients.rows.map(row => row.email), subject, text: sections.map(([title, content]) => `${title}\n${content || 'Sin información'}`).join('\n\n'), html });
