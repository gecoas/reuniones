...
const readBinary = request => new Promise((resolve, reject) => { const chunks = []; let size = 0; request.on('data', chunk => { size += chunk.length; if (size > 25 * 1024 * 1024) { reject(new Error('Audio demasiado grande')); request.destroy(); return; } chunks.push(chunk); }); request.on('end', () => resolve(Buffer.concat(chunks))); request.on('error', reject); });
const escapeEmailHtml = value => String(value || '').replace(/[&<>"']/g, character => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[character])).replace(/\n/g, '<br>');
const audioMimeType = request => { const supplied = String(request.headers['content-type'] || '').split(';')[0]; if (supplied === 'audio/x-m4a') return 'audio/mp4'; if (supplied.startsWith('audio/')) return supplied; const extension = String(request.headers['x-file-name'] || '').split('.').pop().toLowerCase(); return ({ mp3: 'audio/mpeg', m4a: 'audio/mp4', mp4: 'audio/mp4', wav: 'audio/wav', webm: 'audio/webm', ogg: 'audio/ogg' })[extension] || null; };
...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.url?.match(/^\/api\/meetings\/[^/]+\/transcribe$/) && request.method === 'POST') {
      const session = requireSession(request, response); if (!session) return;
      ...
      if (!process.env.GEMINI_API_KEY) return json(response, 503, { error: 'gemini_not_configured' }); const mimeType = audioMimeType(request); if (!mimeType) return json(response, 400, { error: 'unsupported_audio_format' }); const audio = await readBinary(request); if (!audio.length) return json(response, 400, { error: 'audio_required' }); console.log(`Procesando audio de acta: ${mimeType}, ${audio.length} bytes`);
      const key = process.env.GEMINI_API_KEY; const filename = decodeURIComponent(request.headers['x-file-name'] || 'reunion-audio'); const start = await fetch(`https://generativelanguage.googleapis.com/upload/v1beta/files?key=${key}`, { method: 'POST', headers: { 'X-Goog-Upload-Protocol': 'resumable', 'X-Goog-Upload-Command': 'start', 'X-Goog-Upload-Header-Content-Length': String(audio.length), 'X-Goog-Upload-Header-Content-Type': mimeType, 'Content-Type': 'application/json' }, body: JSON.stringify({ file: { display_name: filename } }) }); if (!start.ok) throw new Error(`Gemini upload start ${start.status}`); const uploadUrl = start.headers.get('x-goog-upload-url'); const uploaded = await fetch(uploadUrl, { method: 'POST', headers: { 'X-Goog-Upload-Offset': '0', 'X-Goog-Upload-Command': 'upload, finalize', 'Content-Length': String(audio.length) }, body: audio }); if (!uploaded.ok) throw new Error(`Gemini upload ${uploaded.status}`); let file = (await uploaded.json()).file;
      for (let attempt = 0; attempt < 30 && file.state === 'PROCESSING'; attempt += 1) { await new Promise(resolve => setTimeout(resolve, 2000)); const poll = await fetch(`https://generativelanguage.googleapis.com/v1beta/${file.name}?key=${key}`); if (!poll.ok) throw new Error(`Gemini processing ${poll.status}`); file = (await poll.json()).file; }
      if (file.state !== 'ACTIVE') throw new Error('Gemini no pudo procesar el audio'); const prompt = 'Escucha esta reunión y devuelve solamente JSON válido con estas claves: summary (máximo cinco líneas), content (puntos tratados), agreements (acuerdos numerados), pending (pendientes para la próxima reunión). No inventes información. Usa español.'; const generated = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${key}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ file_data: { mime_type: file.mime_type, file_uri: file.uri } }, { text: prompt }] }], generationConfig: { responseMimeType: 'application/json' } }) }); await fetch(`https://generativelanguage.googleapis.com/v1beta/${file.name}?key=${key}`, { method: 'DELETE' }); if (!generated.ok) throw new Error(`Gemini generation ${generated.status}`); const text = (await generated.json()).candidates?.[0]?.content?.parts?.[0]?.text; return json(response, 200, JSON.parse(text));
    }
    ...
    if (request.url?.match(/^\/api\/minutes\/[^/]+\/send$/) && request.method === 'POST') {
      const session = requireSession(request, response); if (!session) return;
      const id = request.url.split('/')[3]; const minute = await pool.query(`SELECT mi.*, m.department_id, m.title AS meeting_title, m.starts_at, d.name AS department_name FROM minutes mi JOIN meetings m ON m.id = mi.meeting_id JOIN departments d ON d.id = m.department_id WHERE mi.id = $1`, [id]); if (!minute.rows[0] || !await canManageDepartment(session, minute.rows[0].department_id)) return json(response, 403, { error: 'department_manager_required' });
      ...
      const date = new Intl.DateTimeFormat('es-ES', { timeZone: 'Europe/Madrid', day: 'numeric', month: 'numeric', year: '2-digit' }).format(new Date(current.starts_at)); const subject = `Acta reunión Dpto. de ${current.department_name} (${date})`; const sections = [['Asistentes', current.attendees || 'No indicados'], ['Resumen', current.summary], ['Puntos tratados', current.content], ['Acuerdos', current.agreements], ['Pendientes para la próxima', current.pending]]; const html = `<div style="font-family:Arial,sans-serif;line-height:1.5;color:#25282c"><h2>${escapeEmailHtml(subject)}</h2>${sections.map(([title, content]) => `<div style="margin:0 0 18px"><p style="margin:0 0 4px;font-weight:700;font-size:16px"><strong>${title}</strong></p><div>${escapeEmailHtml(content || 'Sin información')}</div></div>`).join('')}</div>`; await transporter.sendMail({ from: SMTP_FROM, to: SMTP_FROM, bcc: recipients.rows.map(row => row.email), subject, text: sections.map(([title, content]) => `${title}\n${content || 'Sin información'}`).join('\n\n'), html });
