...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.url?.match(/^\/api\/meetings\/[^/]+\/transcribe$/) && request.method === 'POST') {
      ...
      if (!process.env.GEMINI_API_KEY) return json(response, 503, { error: 'gemini_not_configured' }); const mimeType = audioMimeType(request); if (!mimeType) return json(response, 400, { error: 'unsupported_audio_format' }); const audio = await readBinary(request); if (!audio.length) return json(response, 400, { error: 'audio_required' });
