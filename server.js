...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.url?.match(/^\/api\/meetings\/[^/]+\/transcribe$/) && request.method === 'POST') {
      ...
      if (!process.env.GEMINI_API_KEY) return json(response, 503, { error: 'gemini_not_configured' }); const mimeType = request.headers['content-type']; if (!mimeType?.startsWith('audio/')) return json(response, 400, { error: 'audio_required' }); const audio = await readBinary(request); if (!audio.length) return json(response, 400, { error: 'audio_required' });
