...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (request.url?.match(/^\/api\/meetings\/[^/]+\/transcribe$/) && request.method === 'POST') {
      ...
      const key = process.env.GEMINI_API_KEY; const start = await fetch(`https://generativelanguage.googleapis.com/upload/v1beta/files?key=${key}`, { method: 'POST', headers: { 'X-Goog-Upload-Protocol': 'resumable', 'X-Goog-Upload-Command': 'start', 'X-Goog-Upload-Header-Content-Length': String(audio.length), 'X-Goog-Upload-Header-Content-Type': mimeType, 'Content-Type': 'application/json' }, body: JSON.stringify({ file: { display_name: request.headers['x-file-name'] || 'reunion-audio' } }) }); if (!start.ok) throw new Error(`Gemini upload start ${start.status}`); const uploadUrl = start.headers.get('x-goog-upload-url'); const uploaded = await fetch(uploadUrl, { method: 'POST', headers: { 'X-Goog-Upload-Offset': '0', 'X-Goog-Upload-Command': 'upload, finalize', 'Content-Length': String(audio.length) }, body: audio }); if (!uploaded.ok) throw new Error(`Gemini upload ${uploaded.status}`); let file = (await uploaded.json()).file;
